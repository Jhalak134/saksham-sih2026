...
import { GlobalAISearchModal } from "@/components/layout/GlobalAISearchModal";
...
function SearchBar({ onAskAI }: SearchBarProps): React.JSX.Element {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const isQuestion =
      trimmed.includes("?") ||
      /^(what|how|why|where|when|who|can|is|are|tell|explain|advice|query|failing|general)/i.test(trimmed);

    if (isQuestion && onAskAI) {
      onAskAI(query.trim());
      return;
    }
...
export function Header(): React.JSX.Element {
  const { openDrawer, language } = useShell();
  ...
  return (
    <>
      ...
      >
        ...
        {/* Center: Exclusive, fully functioning search bar */}
        <div className="flex flex-1 items-center justify-center px-2 md:px-6">
          <SearchBar onAskAI={executeSearch} />
        </div>
